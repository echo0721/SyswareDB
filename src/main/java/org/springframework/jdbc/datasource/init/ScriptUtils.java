...
//				String statement1 =bigSql.toString();
				if((statement1.trim().startsWith("create ")||statement1.trim().startsWith("CREATE "))  
					...
					//文件中只包含单个的执行语句。
					int countOccurrencesOf = StringUtils.countOccurrencesOf(statement1.toLowerCase(), "create ");
		...
		try {
			...
			if(ex.getMessage().contains("无效字符")){
				...
				try {
					...
					logger.debug(ScriptStatementFailedException.buildErrorMessage(statement, 0, resource), e);
				}
			}else{
				boolean dropStatement = StringUtils.startsWithIgnoreCase(statement.trim(), "drop");
				if ( (dropStatement )) {
